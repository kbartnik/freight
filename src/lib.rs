...
pub fn build() -> Result<()> {
    ...
    match (lib_rs.exists(), main_rs.exists()) {
        ...
        (false, false) => return Err("There is noting to compile".into()),
    }
...
pub fn build_tests() -> Result<()> {
    ...
    let main_rs = root_dir.join("src").join("main.rs");
    let target = root_dir.join("target");
    let target_test = target.join("debug").join("tests");
    fs::create_dir_all(&target_test)?;

    match (lib_rs.exists(), main_rs.exists()) {
        (true, true) => {
            test_compile(&manifest, &lib_rs, &target_test, &[])?;
            lib_compile(&mut logger, &manifest, &lib_rs, &target_test)?;
            test_compile(&manifest, &main_rs, &target_test, &[&manifest.crate_name])?;
        }
        (true, false) => {
            test_compile(&manifest, &lib_rs, &target_test, &[])?;
        }
        (false, true) => {
            test_compile(&manifest, &main_rs, &target_test, &[])?;
        }
